...
├── styles/               # PDFの見た目のCSS
│   ├── document.css          # 標準スタイル(キットのファイル。直接編集しない)
│   ├── custom.css            # 自分の見た目調整はここに書く(アップデートで上書きされない)
...
`[生成物]` のフォルダは手で編集せず、Gitにもコミットしません(`.gitignore` で除外済み)。普段編集するのは `documents/` 配下と、見た目を変えるときの `styles/custom.css` だけです(キットのアップデートは [manuals/UPDATE.md](manuals/UPDATE.md) を参照)。文書フォルダの場所を `documents/` から変える方法は [manuals/OPTIONS.md](manuals/OPTIONS.md) の「文書フォルダの場所を変える」を参照してください。
...
| **自分のファイル(アップデートで上書きしない)** | `styles/custom.css` | 見た目の調整はここに書きます。`"styles"` の指定にかかわらず常に最後に読み込まれます。使わなければ削除できます |
| **変更・改名可** | `styles/` のその他のCSS(`document.css`、`header-table.css`、`themes/`) | キットのファイルです。直接編集せず `custom.css` で上書きすると、アップデート時に差し替えるだけで済みます。`document.config.json` の `"styles"` で別名・追加も可能ですが、未指定の文書は `styles/document.css` を使うため、このファイルだけは既定名のまま残してください |
...
| [manuals/UPDATE.md](manuals/UPDATE.md) | 新しいバージョンが出たら | キットのアップデート(上書きしてよいファイルと手順) |
