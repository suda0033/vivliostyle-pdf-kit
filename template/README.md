...
### レイアウトを自分で組みたい場合(上級者向け)

ファイル内に `<section` タグを1つでも書くと、そのファイルでは章の自動生成が無効になり、手書きの構造がそのまま使われます。章にしたい部分は自分で `<section class="chapter">` 〜 `</section>` で囲んでください(番号なしの章は `class="chapter unnumbered"`)。表紙ファイルはこの仕組みで動いています。
