...
貼り込みにあたって、次の3点を調整しています。

- **固有サイズの付与** — `mmdc` が出力するSVGのルート要素は `width="100%"` で高さ属性を持たず、そのままでは画像が固有サイズを持たないため、小さな図でも本文の幅いっぱいに拡大されてしまいます。ビルド時に `viewBox` の値を `width` / `height` 属性として書き込み、本来の大きさで配置されるようにしています。
- **高さの上限** — 図は `break-inside: avoid` によりページ分割されないため、高さに上限がないとページからはみ出した部分が警告なく切り捨てられます。スタイルシート側で `max-height: 75vh`(`vh` は組版領域の高さ)を指定し、はみ出す図は縮小して1ページに収めます。上限を組版領域の75%にしているのは、章見出し・小見出し・導入の段落と図が同じページに入る余地を残すためです(100%近くにすると、見出しだけのページの次に図だけのページができてしまいます)。図の大きさの目安は [USAGE.md](USAGE.md) の「Mermaid図が大きくなりすぎないようにする」を参照してください。
- **キャプションの抑制** — `![alt](...)` と書くとVFMが `alt` をそのまま `<figcaption>` に複製し、図の下に説明文が印字されます。Mermaid図では `<figure><img ...></figure>` を直接出力してこれを避けています。
...
組版に使うCSSは `document.config.json` の `"styles"`(未指定なら `styles/document.css`)です。これに加えて、`styles/custom.css` が存在すれば `vivliostyle.config.js` が**常に末尾に追加**します(`"styles"` に自分で含めている場合は二重に追加しません)。後に読み込まれたCSSが同じセレクタの値を上書きするため、利用者の調整を `custom.css` に集めておけば、キットの `document.css` やテーマCSSをアップデートで差し替えても調整が残ります([UPDATE.md](UPDATE.md) 参照)。`custom.css` を削除した場合は何も追加されません。
