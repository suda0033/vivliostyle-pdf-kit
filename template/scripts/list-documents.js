// documents/ 配下の文書名を列挙し、1行1名で出力する(build-pdf の一括ビルド用)。
...
if (!fs.existsSync(documentsDir)) {
  console.error('エラー: documents/ フォルダがありません。documents/<文書名>/document.config.json を作成してください。');
  process.exit(1);
}
...
if (names.length === 0) {
  console.error('エラー: documents/ に文書がありません(documents/<文書名>/document.config.json が見つかりません)。');
  process.exit(1);
}

const outputs = new Map();
for (const name of names) {
  const configFile = `documents/${name}/document.config.json`;
